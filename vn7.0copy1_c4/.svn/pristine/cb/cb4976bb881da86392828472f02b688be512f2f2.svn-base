...
RESOURCE_MONITORING_JOBS = {
    'meto': ['atmos-xc40_cce_um_fast_omp-seukv-4x9-noios-2t', ],
    'ecmwf': [],
    'nci': [],
    'bom': [],
    'uoe': [],
...
class SuiteReport(object):
    ...
    def generate_task_table(data, omit_housekeeping=True,
        ...
        for task, state in sorted(list(data.items()), key=key_by_name_or_status):
            ...
            for extra_care_string in HIGHLIGHT_ROSE_ANA_FAILS:
                if extra_care_string in task and 'rose_ana' in task:
                    highlight_start = "'''[[span(style=color: #FF00FF, *****"
                    highlight_end = "***** )]]'''"
            if 'succeeded' in state:
                highlight_start = ""
                highlight_end = ""
            lines.append(" || {0:s} || {2:s}{1:s}{3:s} || ".format(task,
                                      state, highlight_start, highlight_end))
        return lines
            ...
            if "working copy changes" in proj_dict:
                if proj_dict["working copy changes"]:
                    ...
                    wc_link = (r"{0:s}/{1:s}/{2:s}/{3:s}?path=log/{4:s}"
                              .format(CYLC_REVIEW_URL[self.site], 'view',
    ...
    def print_report(self):
        ...
        try:
            ...
            trac_log.append(" || Cylc-Review: || {0:s}/{1:s}/{2:s}/{3:s} || ".
               format(CYLC_REVIEW_URL[self.site], 'taskjobs', self.suite_owner,
                      self.suitename))
