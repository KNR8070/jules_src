...
!-----------------------------------------------------------------------------
! "Global" arrays - dimensioned with global_land_pts.
! These hold "global" (full domain) versions of land_pts variables.
!-----------------------------------------------------------------------------
...
SUBROUTINE water_resources_control(                                            &
             land_index, con_rain_ij, con_snow_ij,                             &
...
USE jules_water_resources_mod, ONLY:                                           &
  l_water_irrigation, nstep_water_res, nwater_use, use_irrigation,             &
  water_res_count
...
IF ( l_water_res_call ) THEN
  ...
  !---------------------------------------------------------------------------
  ! Depending on the configuration the water resource code can be run
  ...
  !---------------------------------------------------------------------------
  CALL gather_global_water( demand_accum, irrig_eff )
  ...
  IF ( is_master_task() ) THEN
    CALL water_resources_drive( global_land_pts, demand_accum_global,          &
                                irrig_eff_global, supply_irrig_global )
  END IF

  !---------------------------------------------------------------------------
  ! Scatter information across tasks.
  !---------------------------------------------------------------------------
  CALL scatter_global_water( supply_irrig )
...
IF ( l_allocate ) THEN
  ...
  !---------------------------------------------------------------------------
  ! Allocate arrays.
  ! These are allocated at full size only on the master task.
  !---------------------------------------------------------------------------
  IF ( is_master_task() ) THEN
  ...
  ALLOCATE(irrig_eff_global(land_size_irrig), STAT = ERROR)
  error_sum = error_sum + ERROR
...
ELSE
  ...
  IF ( ALLOCATED(irrig_eff_global) )    DEALLOCATE(irrig_eff_global)
...
SUBROUTINE gather_global_water( demand_accum, irrig_eff )
...
IF ( l_water_irrigation ) THEN
  CALL gather_land_field( irrig_eff, irrig_eff_global )
END IF
...
SUBROUTINE scatter_global_water( supply_irrig )
...
!-----------------------------------------------------------------------------
! Description:
!   Scatter a global land field across tasks.
!-----------------------------------------------------------------------------
