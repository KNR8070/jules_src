...
TYPE :: file_ts
  ...
  INTEGER :: data_period ! The period of the data in the files
                         ! (in seconds or the 'special' periods period_month and
                         !  period_year).
...
USE datetime_mod, ONLY: period_day, period_month, period_year, datetime_advance
...
CASE DEFAULT
  CALL log_fatal("file_ts_open",                                               &
                 TRIM(fname) // ": " //                                        &
                 "Data period must be > 0 or a 'special' period (1 month " //  &
                 "or 1 year) (supplied " // TRIM(to_string(data_period))   //  &
                 ")")
END SELECT
...
IF ( use_time_template ) THEN
  ...
  CASE ( period_day, period_month, period_year )
...
!-----------------------------------------------------------------------------
! Argument types
TYPE(file_ts), INTENT(IN OUT) :: FILE
...
USE datetime_mod, ONLY: period_day, period_month, period_year, l_360, l_leap,  &
...
!-----------------------------------------------------------------------------
! Argument types
TYPE(file_ts), INTENT(IN OUT) :: FILE  ! The file to seek
...
! Note there is no consideration of period_day as that is not allowed as a
! data period (use 86400 instead).
...
IF ( FILE%use_time_template ) THEN
  ...
  CASE ( period_day )
    ! The first data in a daily file must apply at 00:00:00 on that day.
    file_start = datetime_create(dt_local%year, dt_local%month, dt_local%day,  &
                                 0, 0, 0)

  CASE ( period_month )
    ! The first data in a monthly file must apply at 00:00:00 on the 1st of the
    ! month
    file_start = datetime_create(dt_local%year, dt_local%month, 1, 0, 0, 0)
...
TYPE(data_cube), INTENT(IN) :: cube
                               ...
                               ! be the grid dimensions OR the first 1
                               ! dimension should be the vector dimension
...
!-----------------------------------------------------------------------------
! Argument types
TYPE(file_ts), INTENT(IN OUT) :: FILE ! The file to close
...
!-----------------------------------------------------------------------------
! Argument types
TYPE(file_ts), INTENT(IN OUT) :: FILE  ! The file to update
...
TYPE(data_cube) :: temp  ! Temporary cube to read data in case we need to
                         ! collapse dimensions
    ...
    IF ( n_non_time_vars > 0 ) THEN
      ...
      DO i = 1,FILE%nvars
        IF ( .NOT. FILE%vars(i)%use_time ) THEN
          ...
          ! Read into a temporary cube, which is always a 2d grid
          ! We are not using a subgrid, so give extract_subgrid=F
          temp = file_ts_read_var(FILE, i, .FALSE.)

          IF ( FILE%grid%is_1d ) THEN
            ! If necessary, collapse the degenerate y dimension to get
            ! a cube with a 1d grid
            ! We know that this is dimension number 2
            CALL cube_collapse_dimension(non_time_varying_data(j), temp, 2)
          ELSE
            ! Otherwise, just copy the cube
            CALL cube_safe_copy(non_time_varying_data(j), temp)
          END IF
