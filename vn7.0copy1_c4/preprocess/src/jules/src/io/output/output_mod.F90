...
TYPE :: output_profile
  ...
  LOGICAL :: is_river_profile = .FALSE.  ! Indicates if this profile is for
                                         ! variables on the river grid.
  ...
  LOGICAL :: l_land_frac = .FALSE. ! Include land_fraction data
...
TYPE(grid_info), SAVE :: river_out_grid ! The river output grid
                                        ! Used if any river variables are
...
! Variables
    output_dir, run_id, grid, use_subgrid, subgrid, river_out_grid,            &
    dump_period, dump_period_unit,                                             &
! Routines
...
DO i = 1,nprofiles
  ...
  CASE ( format_ascii )
    IF ( profiles(i)%is_river_profile ) THEN
      CALL log_fatal("output_initial_data",                                    &
                     "Cannot create river grid output in ASCII format")
    END IF
  ...
  FILE=internal_open_output_file(                                              &
    file_start, current_time, 1, .FALSE., profiles(i)%is_river_profile,        &
    file_name, out_grid, profiles(i)%fields(1:profiles(i)%nfields),            &
    profiles(i)%l_land_frac                                                    &
  )
  ...
  DO j = 1,profiles(i)%nfields
    ...
    IF ( profiles(i)%is_river_profile ) THEN
      ! River profiles are written by the master task only
      IF ( is_master_task() ) THEN
        ...
        CALL file_ts_write_var(                                                &
           ...
           ! No subgrid used for river variables
           .FALSE.                                                             &
           )

      END IF
    ELSE
      ! All other variables are written in parallel
  ...
  !-----------------------------------------------------------------------------
  ! Close the file. We do this for all tasks of a land profile and just the
  ! master task of a river profile.
  !-----------------------------------------------------------------------------
  IF ((.NOT. profiles(i)%is_river_profile) .OR. is_master_task())              &
...
SUBROUTINE register_output_profile(profile_name, output_initial,               &
                                   ...
                                   l_land_frac, output_start, output_end,      &
                                   output_period, sample_period, file_period,  &
                                   identifiers, var_names, output_types)

USE datetime_mod, ONLY: period_day, period_month, period_year, secs_in_day,    &
                        datetime_to_string

USE model_time_mod, ONLY: max_spinup_cycles, main_run_start, main_run_end,     &
                           timestep_len

USE model_interface_mod, ONLY: get_var_id, is_river_var
...
LOGICAL, INTENT(IN) :: l_land_frac
    ! T - Gridbox land fraction will be output
...
LOGICAL :: have_river_variable ! Flag indicating that a river point
...
! Check the given periods.
IF ( file_period /= period_day .AND. file_period /= period_month               &
     .AND. file_period /= period_year ) THEN
...
! Check that output_period is not period_day.
IF ( output_period == period_day ) THEN
  CALL log_fatal("register_output_profile",                                    &
                 "Output period cannot be the special period period_day. " //  &
                 "Use output period = 86400 seconds to select daily data.")
END IF

IF ( output_period /= period_month .AND. output_period /= period_year .AND.    &
     MOD(output_period, timestep_len) /= 0 )                                   &
! If output period is not a special period (monthly or yearly), it must be a
! multiple of the model timestep.
    CALL log_fatal("register_output_profile",                                  &
...
!
! Finally, if the river and land grids differ, we check whether variables are
! river or land variables, and define the output grid accordingly. We only
! allow one grid type in each profile.
!-----------------------------------------------------------------------------

have_river_variable = .FALSE.
...
DO i = 1,nvars
  ...
  ! A profile cannot contain both land point and river point variables as these
  ! are on different grids.
  ! Note that this is also the first point at which an unknown identifier will
  ! be detected.
  IF ( is_river_var(identifiers(i)) ) THEN
    IF ( .NOT. l_rivers ) THEN
      CALL log_fatal("register_output_profile",                                &
                     "Cannot request output on river grid if " //              &
                     "l_rivers=.FALSE." )
    END IF
    IF ( i == 1 ) THEN
      have_river_variable = .TRUE.
    ELSE IF ( .NOT. have_river_variable ) THEN
      ! This is a river variable, but we have previously found a non-river var.
      CALL log_fatal("register_output_profile",                                &
                     "Cannot mix land and river grid variables in an " //      &
                     "output file. Variable=" // TRIM(identifiers(i)) //       &
                     " in profile " // TRIM(profile_name))
    END IF
  ELSE IF ( have_river_variable ) THEN
    ! This is not a river variable, but we have previously found a river var.
    ! This is a river variable, but we have previously found a non-river var.
    CALL log_fatal("register_output_profile",                                  &
                   "Cannot mix land and river grid variables in an " //        &
                   "output file. Variable=" // TRIM(identifiers(i)) //         &
...
profiles(nprofiles)%is_river_profile = have_river_variable
profiles(nprofiles)%l_land_frac      = l_land_frac
...
DO i = 1,nprofiles
  ...
  IF ( next_time >= profiles(i)%next_output_time ) THEN
    ...
    ! and if this task has an open file
    ! If it is not, then advance the file
    IF ( ( profiles(i)%has_open_file ) .AND.                                   &
      ( profiles(i)%current_output_time < profiles(i)%fh%data_end ) ) THEN
...
DO i = 1,nprofiles
  ...
  ! Close the file
  ! For river files this is done by the master task, for all other files it is
  ! done by all tasks
...
!-----------------------------------------------------------------------------
! Argument types
TYPE(output_profile), INTENT(IN OUT) :: profile  ! The profile to initialise
                                                ! fields for


! Work variables
INTEGER, ALLOCATABLE :: grid_shape(:)  ! The size of the grid dimension(s)
                                       ! For river variables, only one
                                       ! dimension is required. For all
                                       ! others, two dimensions are
                                       ! required, and their size depends
...
IF ( profile%is_river_profile ) THEN
  ALLOCATE(grid_shape(1))
  grid_shape(:) = np_rivers
ELSE
  ALLOCATE(grid_shape(2))
  IF ( use_subgrid ) THEN
...
!-----------------------------------------------------------------------------
! Argument types
TYPE(file_ts), INTENT(IN OUT) :: FILE  ! The file to define variables on
TYPE(dict), INTENT(IN OUT) :: defined_dims
