...
,t_growth(land_pts)                                                            &
                            ! WORK growth temperature at one PFT
,co2_growth(land_pts)                                                          &
...
alpha_acclim = 1.0 - EXP( -1.0 * timestep_real                                  &
                                / ( n_day_photo_acclim * 3600.0 ) )
    IF (current_time_pmodel == 41400 .or. current_time_pmodel == 43200         &
       .or. current_time_pmodel == 45000) THEN
      t_growth(l) = t_growth_gb(l,n)                                   &
                     + alpha_acclim * ( t1_land(l) - t_growth_gb(l,n) )
      t_growth_gb(l,n) = t_growth(l)
    print*,'tg',t_growth(l),'pft',n
    END IF
  END DO
