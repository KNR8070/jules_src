...
USE jules_surface_mod, ONLY: h_blend_max, fd_stability_dep, on, orog_drag_param
...
!$OMP        ho2r2_orog,fd_stability_dep,rib,z0m_eff,                          &
...
DO k = 1,surft_pts
  ...
  IF ( rib(l) >  ri_crit .AND.                                                 &
       fd_stability_dep == on )  z0m_eff(l) = z0m(l)
